...
47th version - Added Active and Inactive Fisher Folk registry Edit and Print actions, loaded the selected record into the reference application form, restored saved checkbox/radio selections, updated existing applications without duplicates, and opened the corrected form directly for printing.
48th version - Fixed Fishery Application print output so the existing form HTML and CSS are printed directly, removed the duplicate print template, and prevented missing saved values from displaying as "undefined".
49th version - Matched the visible application form more closely to the supplied paper layout by removing duplicated option labels and moving Save application and Print application controls outside the paper while preserving form submission.
50th version - Separated Fishery Application presentation logic into dedicated CSS and JavaScript assets, leaving the PHP page responsible for data/configuration and existing form markup without changing database queries or backend functionality.
